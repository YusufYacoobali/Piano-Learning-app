import 'helper_list.dart';
import 'helper_note_info.dart';

/// A list that contains all information about helper of bass.
final HelperList bassNoteImageNameList = HelperList(
  helperId: 1,
  helperList: [
    HelperNoteInfo(
        noteImageName: 'assets/note_images/Bs_A.jpeg',
        noteName: 'Bass A',
        noteSound: 'note_a.wav'),
    HelperNoteInfo(
        noteImageName: 'assets/note_images/Bs_B.jpeg',
        noteName: 'Bass B',
        noteSound: 'note_b.wav'),
    HelperNoteInfo(
        noteImageName: 'assets/note_images/Bs_C.jpeg',
        noteName: 'Bass C',
        noteSound: 'note_c.wav'),
    HelperNoteInfo(
        noteImageName: 'assets/note_images/Bs_D.jpeg',
        noteName: 'Bass D',
        noteSound: 'note_d.wav'),
    HelperNoteInfo(
        noteImageName: 'assets/note_images/Bs_E.jpeg',
        noteName: 'Bass E',
        noteSound: 'note_e.wav'),
    HelperNoteInfo(
        noteImageName: 'assets/note_images/Bs_F.jpeg',
        noteName: 'Bass F',
        noteSound: 'note_f.wav'),
    HelperNoteInfo(
        noteImageName: 'assets/note_images/Bs_G.jpeg',
        noteName: 'Bass G',
        noteSound: 'note_g.wav'),
  ],
);

/*
  static String getImagePath(index) {
    return bassNoteImageNameList[index].noteImageName;
  }

  static String getImageName(index) {
    return bassNoteImageNameList[index].noteName;
  }

  static String getNoteSound(index) {
    return bassNoteImageNameList[index].noteSound;
  }
  */
